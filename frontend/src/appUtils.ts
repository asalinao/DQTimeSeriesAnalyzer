...
export function mergePoints(previous: SeriesPoint[], next: SeriesPoint[]): SeriesPoint[] {
  ...
  next.forEach((point) => byId.set(point.id, point));
  return [...byId.values()].sort((left, right) => new Date(left.timestamp).getTime() - new Date(right.timestamp).getTime());
}
...
export function formatAnomalyDeviation(anomaly: Anomaly): string {
  ...
  if (typeof anomaly.relative_deviation === "number") {
    parts.push(
      ...
      }).format(anomaly.relative_deviation * 100) + "%",
    );
  }
...
export function scheduleSummary(monitor: Monitor): string {
  ...
  const unit = scheduleTypeLabels[monitor.schedule_type] ?? "минут";
  return `Запуск каждые ${monitor.schedule_value} ${unit}`;
}
