...
def generate_rows(
    ...
    for offset in range(hours):
        bucket = start_at + timedelta(hours=offset)
        for _ in range(order_count(bucket, base_rate, mode)):
            ...
            rows.append(OrderRow(created_at, choose_amount(created_at, status, mode), status, choose_customer(created_at, mode)))
    return sorted(rows, key=lambda row: row.created_at)


def order_count(ts: datetime, base_rate: float, mode: str) -> int:
    hour = ts.hour
    business_peak = 0.65 * bell(hour, 13, 3.3)
    evening_peak = 0.45 * bell(hour, 20, 2.2)
    night_factor = 0.35 if 0 <= hour <= 5 else 1.0
    weekend_factor = 0.72 if ts.weekday() >= 5 else 1.0
    monday_factor = 1.12 if ts.weekday() == 0 else 1.0
...
def choose_status(mode: str) -> str:
    weights = (0.10, 0.42, 0.08, 0.08, 0.02, 0.30) if mode == "payment_failures" else (0.12, 0.62, 0.16, 0.05, 0.03, 0.02)
...
def choose_customer(ts: datetime, mode: str) -> int:
    ...
    return random.randint(1, 80) if random.random() < 0.18 else random.randint(1, max(100, active_pool))
...
def choose_amount(ts: datetime, status: str, mode: str) -> Decimal | None:
    ...
    if mode == "amount_shift":
        ...
        median_amount *= 0.28
    value = random.lognormvariate(math.log(median_amount), 0.42)
    if random.random() < 0.025:
        value *= random.uniform(2.5, 6.0)
    return Decimal(str(round(max(1.0, min(5000.0, value)), 2)))


def bell(hour: int, center: int, width: float) -> float:
